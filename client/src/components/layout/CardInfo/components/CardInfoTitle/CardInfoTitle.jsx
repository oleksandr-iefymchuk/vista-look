...
const CardInfoTitle = ({ _id, productCode, title, quantity }) => {
  ...
  const getReviewsWordForm = (count) => {
    ...
    return ['відгук', 'відгуки', 'відгуків'][count % 100 > 4 && count % 100 < 20 ? 2 : cases[count % 10 < 5 ? count % 10 : 5]];
  };
          ...
          <Fragment>
            ...
            <div className='share-popup'>
              <div className='share-popup-header'>
                <h3>Поділитися</h3>
                <ButtonWrapper buttonClassName='close-popup-btn' icon='close' onClick={togglePopup} />
              </div>
      ...
      <div className='product-code'>
        ...
        <div className='reviews'>
          {productReviews.length > 0 ? (
            <Fragment>
              <Rating className='rating' value={calculateAverageRating(productReviews)} precision={0.5} readOnly />
              <p className='link-comment' onClick={scrollToReviews}>
                {`${productReviews.length} ${getReviewsWordForm(productReviews.length)}`}
              </p>
            </Fragment>
          ) : (
            <p className='link-comment' onClick={scrollToReviews}>{`Написати відгук`}</p>
          )}
        </div>
        <ButtonWrapper buttonClassName='share-btn' icon='share' onClick={togglePopup} />
      </div>
    </div>
  );
};
