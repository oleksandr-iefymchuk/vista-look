      ...
      <div className='basket'>
        ...
        <div className='basket-list'>
          {basket.map(({ productId, quantity, size, productCode }) => {
            ...
            return product ? (
              <BasketItem key={productId} {...product} quantity={quantity} size={size} productCode={productCode} />
            ) : null;
          })}
        </div>
        {basket.length > 0 ? (
          <div className='total-amount-block'>
            <TotalPrice />
            <ButtonWrapper buttonClassName='order-btn' buttonText='Перейти до оформлення' onClick={handleOrder} />
          </div>
        ) : (
          <div className='empty-basket'>
            <SvgIcon name='empty-basket' color='#a2a2a2' width='300px' height='200px' />
